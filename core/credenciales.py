"""Almacenamiento local y cifrado de las credenciales del RPA (SIPP).

Las credenciales se guardan en un archivo JSON en la carpeta de datos del
usuario. La contraseña nunca se guarda en claro: se cifra con DPAPI (ver
core/dpapi.py), que ata el cifrado a la cuenta de usuario de Windows (solo el
mismo usuario en la misma máquina puede descifrarla, sin que la app maneje
ninguna llave propia).
"""
